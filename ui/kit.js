...
const CSS = `
.san-main { container-type: inline-size; }
.san-row3 { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: var(--sy-s3); }
.san-row2 { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: var(--sy-s3); }
.san-ask { display: grid; grid-template-columns: minmax(0, 1fr) minmax(260px, 320px); grid-template-rows: minmax(0, 1fr); gap: var(--sy-s3); }
.san-item { display: grid; grid-template-columns: minmax(0, 1fr) minmax(300px, 380px); gap: var(--sy-s3); align-items: start; }
.san-prose h1, .san-prose h2, .san-prose h3, .san-prose h4 { margin: 0.6em 0 0.3em; line-height: 1.25; }
.san-prose p { margin: 0.4em 0; }
.san-prose ul, .san-prose ol { margin: 0.4em 0; padding-left: 1.4em; }
.san-prose blockquote { margin: 0.5em 0; padding-left: 0.8em; border-left: 3px solid var(--sy-line-strong); color: var(--sy-text-2); }
.san-prose code { font-family: var(--sy-mono, monospace); font-size: 0.92em; }
.san-prose a { color: var(--sy-brass); }
@container (max-width: 620px) {
  .san-ask { grid-template-columns: minmax(0, 1fr); grid-template-rows: none; height: auto !important; }
}
@container (max-width: 700px) {
  .san-row3 { grid-template-columns: minmax(0, 1fr); }
  .san-item { grid-template-columns: minmax(0, 1fr); }
}
@container (max-width: 640px) {
  .san-row2 { grid-template-columns: minmax(0, 1fr); }
}
`;

/** The plugin's own layout rules, added to the document the first time a panel is drawn. */
export function ensureStyles() {
  if (typeof document === 'undefined' || document.getElementById('san-kit-css')) return;
