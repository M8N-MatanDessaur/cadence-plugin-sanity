<#
.SYNOPSIS
    Documents of every type whose title, slug or id contains the text.
.EXAMPLE
    ./scripts/Search-Documents.ps1 -Query launch
#>
[CmdletBinding()]
param(
...
Out-Json @((Get-Api "/api/plugins/sanity/entries?q=$(Esc $Query)&limit=$Limit").entries) 6
