<#
.SYNOPSIS
    Writes a whole document from a JSON file as its draft (what the Studio does when you edit); -Publish writes the published version instead.
.EXAMPLE
    ./scripts/Save-Document.ps1 -Id press-article-1 -JsonFile .ai-workspace/doc.json
#>
[CmdletBinding()]
param(
...
Post-Api '/api/plugins/sanity/save' @{ id = $Id; doc = $doc; publish = [bool]$Publish } | ConvertTo-Json
