<#
.SYNOPSIS
    Runs a GROQ query (with -Params as JSON for $name parameters) and prints the result as JSON.
.EXAMPLE
    ./scripts/Run-Groq.ps1 -Query '*[_type == "post"]{title}'
#>
[CmdletBinding()]
param(
...
$payload = @{ query = $Query; meta = $true }
if ($Params) { $payload.params = ConvertFrom-Json -InputObject $Params }
$r = Post-Api '/api/plugins/sanity/groq' $payload
Out-Json $r.result 30
